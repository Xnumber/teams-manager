#pragma once
inline const char *getParticipatingProjectTaskStatsSql = R"(
WITH month_tasks AS (
	SELECT
		t.id,
		t.project_id
	FROM tasks t
	WHERE t.created_at >= date_trunc('month', $2::date)
	  AND t.created_at < (date_trunc('month', $2::date) + interval '1 month')
