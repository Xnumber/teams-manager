...
export class TasksDataSourceService {
  ...
  createInCompletedTasksThisMonthDataSource(userId: string): DataSource<JBTask> {
    return new DataSource({
      load: () => {
        let url = '/completed-this-month-tasks-by-user';
        const params: any = {};
   ...
   */
  createParticipatingProjectByMonthAndUserDataSource(
    userId: string,
    month: string
  ): DataSource<{ project_id: string; project_name: string; count: number; }> {
    return new DataSource<{ project_id: string; project_name: string; count: number; }>({
      load: () => {
        const url = '/participating-projects-by-month-and-user';
        const params: any = { user_id: userId, month };
  ...
  createCompletedTasksByMonthDataSource(userId: string, month: string): DataSource<JBTask> {
    return new DataSource<JBTask>({
      load: () => {
        const url = '/task-completed-by-month-and-user';
        const params: any = {
          user_id: userId,
          month,
        };
