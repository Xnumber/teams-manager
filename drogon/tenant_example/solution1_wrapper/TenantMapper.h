 ...
 * 使用示例：
 * @code
 * TenantMapper<Projects> mapper(dbClient, tenantId);
...
public:
     ...
     */
    TenantMapper(DbClientPtr client): mapper_(client)
    // ,tenantId_(tenantId)
    {
        tenantId_ = RequestContext::getTenantId();
        LOG_DEBUG << "TenantMapper constructor - Tenant ID from RequestContext: " << tenantId_;
        userId_ = RequestContext::getUserId();
        LOG_DEBUG << "TenantMapper constructor - User ID from RequestContext: " << userId_;
        client_ = client;
    }
     ...
     * @brief 設置查詢限制
     * @param limit 限制數量
     * @return TenantMapper 引用（支持鏈式調用）
     */
    TenantMapper<T> &limit(size_t limit)
    {
        mapper_.limit(limit);
        return *this;
    }
     ...
     * @return TenantMapper 引用（支持鏈式調用）
     */
    TenantMapper<T> &offset(size_t offset)
    {
     ...
     * @return TenantMapper 引用（支持鏈式調用）
     */
    TenantMapper<T> &orderBy(const std::string &colName,
     ...
     * @return TenantMapper 引用（支持鏈式調用）
     */
    TenantMapper<T> &orderBy(size_t colIndex,
                             const SortOrder &order = SortOrder::ASC)
    {
     ...
     * @return TenantMapper 引用（支持鏈式調用）
     */
    TenantMapper<T> &paginate(size_t page, size_t perPage)
    {
