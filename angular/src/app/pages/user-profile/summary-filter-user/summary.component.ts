...
import { DataSource, DataSourceOptions } from 'devextreme/common/data';
...
export class UserProfileSummaryComponent {
  ...
  constructor(
    ...
    this.executors = this.tasksDataSourceService.createUserDataSource();
    ...
    //   });
    //   this.participatingProjectThisMonthDataSource = tasksDataSourceService.createParticipatingProjectByMonthAndUserDataSource(
    //     userId,
    //     month
    //   );
  ...
  lookupHistory = () => {
    ...
    forkJoin([
      ...
      const githubWorkLoad = ((githubData['pr_count'] || 0) + commit_count) * this.githubWeightedMap[githubData['repository_count'] || 0];
  ...
  getGitInfoThisMonth() {
    this.http.get<any>(`/statistic-git-info-by-month-and-user?month=${this.year}-${this.month}&user_id=${this.userId}`).subscribe(res => {
      ...
      this.workLoadThisMonth.set(res.work_load || 0);
  ...
  monthChanged = (e: ValueChangedEvent) => {
    if (this.userId) {
      const month = `${this.year}-${this.month}`
      this.completedTasksThisMonthDataSource = this.tasksDataSourceService.createCompletedTasksByMonthDataSource(
        this.userId,
        month
      )
      ...
      this.completedTasksDataSource = this.tasksDataSourceService.createCompletedTasksDataSource(this.userId);
      this.completedTasksDataSource.load().then(data => {
        const totalCount = this.completedTasksDataSource?.totalCount() ?? data.length;
        this.count.set(totalCount);
      });

      this.participatingProjectThisMonthDataSource = this.tasksDataSourceService.createParticipatingProjectByMonthAndUserDataSource(
        this.userId,
        month
      );
      this.participatingProjectThisMonthDataSource.load().then(data => {
      ...
      this.inCompletedTasksDataSource = this.tasksDataSourceService.createInCompletedTasksDataSource(this.userId);
      this.inCompletedTasksDataSource.load().then(data => {
        const totalCount = this.inCompletedTasksDataSource?.totalCount() ?? data.length;
        this.inCompleteCount.set(totalCount);
      });
      this.inCompletedTasksThisMonthDataSource = this.tasksDataSourceService.createInCompletedTasksThisMonthDataSource(this.userId);
      this.inCompletedTasksThisMonthDataSource.load().then(data => {
        const totalCount = this.inCompletedTasksThisMonthDataSource?.totalCount() ?? data.length;
        this.completedCountThisMonth.set(totalCount);
      });
  ...
  yearChanged = (e: string) => {
    const month = `$${e}-${this.month}`
    ...
    this.completedTasksDataSource = this.tasksDataSourceService.createCompletedTasksDataSource(userId);
    this.completedTasksDataSource.load().then(data => {
      const totalCount = this.completedTasksDataSource?.totalCount() ?? data.length;
      this.count.set(totalCount);
    });
    ...
    this.inCompletedTasksDataSource = this.tasksDataSourceService.createInCompletedTasksDataSource(userId);
    this.inCompletedTasksDataSource.load().then(data => {
      const totalCount = this.inCompletedTasksDataSource?.totalCount() ?? data.length;
      this.inCompleteCount.set(totalCount);
    });
    this.inCompletedTasksThisMonthDataSource = this.tasksDataSourceService.createInCompletedTasksThisMonthDataSource(userId);
    this.inCompletedTasksThisMonthDataSource.load().then(data => {
      const totalCount = this.inCompletedTasksThisMonthDataSource?.totalCount() ?? data.length;
      this.completedCountThisMonth.set(totalCount);
    });
