...
import { conditionsToObject } from '../../utils/conditionsToObject';
...
export class TasksDataSourceService {
  ...
  createHistoryTasksDataSource(): DataSource<JBTask> {

    // Optionally implement insert/update/remove for CRUD
    return new DataSource({
      load: (loadOptions) => {
        console.log('History tasks loading with options', loadOptions);
        let url = '/history-tasks';
        const params: any = {};
        if (loadOptions.filter && Array.isArray(loadOptions.filter) && loadOptions.filter.length >= 2) {
          Object.assign(params, conditionsToObject(loadOptions.filter));
          console.log('Filtering history tasks with params', params);
        }
        return lastValueFrom(this.http.get<{ count: number; result: string; tasks: RawTask[] }>(url, { params })
          .pipe(catchError(err => {
            console.error('Error loading next week tasks', err);
