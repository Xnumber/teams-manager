...
std::string findLatestTaskEndDate(
    ...
    for (const auto &item : ganttData) {
        if (item["workItemType"].asString() == "TASK") {
            std::string taskEnd = item["end"].asString();
...
// std::string createBasicTaskGanttItem(const drogon::orm::Row &taskRawData, const std::string &startDate, const std::string &endDate)
// {
//     Json::Value item;
